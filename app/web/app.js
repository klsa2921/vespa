    const App = () => {
      ...
      // New state for Vespa configuration
      const [vespaConfig, setVespaConfig] = useState({});
      const [vespaConfigLoading, setVespaConfigLoading] = useState(false);
      const [vespaConfigError, setVespaConfigError] = useState(null);
      ...
      // Modified to include vespaConfig in payload
      ...
      // Handle Vespa config input changes
      const handleVespaConfigChange = (e) => {
        const { name, value } = e.target;
        setVespaConfig((prev) => ({ ...prev, [name]: value }));
      };
      ...
      return (
        <div className="max-w-4xl mx-auto mt-10 p-6 bg-white rounded-lg shadow-md">
          <div className="menu-bar flex justify-between mb-6">
            <div
              className={`menu-item px-4 py-2 cursor-pointer ${
                currentTab === 'index' ? 'bg-blue-500 text-white rounded-md' : ''
              }`}
            ...
            <div
              className={`menu-item px-4 py-2 cursor-pointer ${
                currentTab === 'search' ? 'bg-blue-500 text-white rounded-md' : ''
              }`}
